...
DEFAULT_SLAVE_Q=
SNAKEFILE=/mnt/projects/wangcl/ancestry/software/pipelines_web_dev/varCall/Snakefile
LOGDIR="logs";# should be same as defined above
...
if [ "$ENVIRONMENT" == "BATCH" ]; then
    ...
    clustercmd="$clustercmd -cwd -v PATH -e $LOGDIR -o $LOGDIR"
    if [ -n "$SLAVE_Q" ]; then
    ...
    CLUSTER_ARGS="--cluster-config cluster.yaml $clustercmd --jobname \"runTopMed.slave.{rulename}.{jobid}.sh\""
    N_ARG="--jobs 25"
else
    # run locally
    CLUSTER_ARGS=""
...
cmd="snakemake $sm_args >> logs/snakemake.log 2>&1"
if [ $DEBUG -eq 1 ]; then
    echo $cmd
else
    eval $cmd
